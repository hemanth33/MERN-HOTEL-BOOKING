...
const Search = () => {
    ...
    const handleHotelTypesChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSelectedHotelTypes((prevHotelTypes) => 
            event.target.checked ? [...prevHotelTypes, hotelType] : prevHotelTypes.filter((type) => type !== hotelType)
        );
    }
    ...
    const handleFacilitiesChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSelectedFacilities((prevFacilities) => 
            event.target.checked ? [...prevFacilities, facility] : prevFacilities.filter((type) => type !== facility)
        );
    }
        ...
        <div className="grid grid-cols-2 lg:grid-cols-[250px_1fr] gap-5">
            <div className="rounded-lg border border-slate-300 p-5 h-fit sticky top-10">
                <div className="space-y-5">
                    <h3 className="text-lg font-semibold border-b border-r-slate-300 pb-5">Filter By</h3>
                    <StarRatingFilter selectedStars={selectedStars} onChange={handleStarsChange}/>
                    <HotelTypesFilter selectedHotelTypes={selectedHotelTypes} onChange={handleHotelTypesChange}/>
                    <FacilitiesFilter selectedFacilityTypes={selectedFacilities} onChange={handleFacilitiesChange}/>
